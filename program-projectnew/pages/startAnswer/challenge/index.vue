<template>
	...
	<template class="wrapper2">
		...
		<view style="position: absolute;width: 100%;height: 100%;display: flex;flex-direction: raw;overflow: auto;z-index: 99;background-color: #fbfbfb;
		...
		<view style="position: absolute;width: 100%;height: 100%;" id="resultPage" v-if="isShowResult" class="wrapper2">
			<view
				style="position: absolute;border: 1px solid black;width: 80%;height: 80%;left: 0;right: 0;top: 0;bottom: 0;margin: auto;">
				<view style="border-radius: 999px;border: 1px solid black;width: 200rpx;height: 200rpx;background-image: url('../../../static/image.png');
			background-repeat: no-repeat;background-size: cover;left: 0;right: 0;margin: auto;transform: translateY(-100rpx);
			">
					<!-- 这是头像 -->
				</view>
				<view style="position: relative;display: flex;">
					<img src="../../../static/烟花.svg" alt=""
						style="width: 100rpx;height: 100rpx;position: absolute;left: 30rpx;">
					<span style="text-align: center;font-size: 70rpx;width: 100%;">挑战完成</span>
				</view>
				<view
					style="display: flex;flex-direction: column;border: 1px solid black;height: 50%;position: relative;">
					<span style="text-align: center;">共{{questionList.length}}题</span>
					<span style="text-align: center;">答对{{correctNumber}}题</span>
					<span style="text-align: center;">答错{{questionList.length-correctNumber}}题</span>
				</view>
				<view style="display: flex;justify-content: space-evenly;">
					<van-button round icon="wap-home" plain color="#fb8c00" @click="gotoHome"></van-button>
					<van-button round plain color="#fb8c00">分享</van-button>
				</view>
			</view>
		</view>
    ...
    " v-if="!isShowResult" class="wrapper2">
      ...
      " class="items" v-if="questionList.length!=0">
		  ...
		  background-image: url(../../../static/书卷.svg);
        ...
        ">
					...
					font-family: '阿里妈妈东方大楷';font-weight:100 ;
					">封神榜</span>
				</view>
				<view style="position: absolute;left: 0;right: 0;bottom: -35rpx;margin: auto;width: 250rpx;text-align: center;
			font-family: '阿里妈妈东方大楷';color: #e64a19;
			">倒计时：{{time}}s</view>
			</view>
			<view style="grid-row-start: 3; grid-row-end: 7; position: relative" class="items"
			...
			<view style="grid-row-start: 3; grid-row-end: 7; position: relative" class="items"
				v-if="questionList.length!=0">
				<view style="position: absolute;width: 250rpx;height: max-content;left: 35rpx;top: -30rpx;
			font-family: 第几题字体;font-size: 50rpx;
			">第{{questionIndex+1}}题</view>
				<view style="position: absolute; top: 30rpx; width: 100%; height: 80%">
					<van-button round color="#f9a825" style="position: absolute; left: 50rpx" size="mini">
      ...
      " class="items" v-if="questionList.length!=0">
		...
		width: 100%;" v-if="questionList[questionIndex].questionClass=='选择题'">
          ...
          " v-for="(item,index) in ['正确','错误']" @click="clickAnswer(item)"
			...
			<view style="grid-row-start: 13; grid-row-end: 16;position: relative;" v-if="questionList.length!=0"
				class="items">
				<view
					...
					<view>
						...
						<van-button round :color="questionPrevColor" @click="goto('prev')"
							class="navigateQuestion">上一题</van-button>
					</view>
					<view>
						<van-button round :color="questionNextColor" @click="goto('next')"
				...
				<view style="position: absolute;left: 0;right: 0;margin: auto auto;bottom: 20rpx;width: max-content;">
					<van-button round color=" #ffc400" @click="submit('主动')">&ensp;&ensp;提交&ensp;&ensp;</van-button>
				</view>
...
<script>
	...
	export default {
		...
		onLoad: function(option) {
			...
			uni.request({
				url: `http://localhost:3000/challenge?username=${this.who}&number=300&name=${this.questionBank}&userId=${this.uid}`,
				success: (res) => {
				...
				if (type == "主动") {
					Dialog.confirm({
						...
						if (this.who != '游客') {//别把这里和下面else里面的这个删掉然后放外面，会有bug，this.correctNumber一直为0
								uni.request({
									url: `http://localhost:3000/challengeResult`,
									method: "POST",
									data: {
										username: this.who,
