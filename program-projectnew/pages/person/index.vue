<template>
	...
	<template v-if="isShowChoosePicture">
		<view style="position: absolute;width: 90%;height: 54%;border: 1px white solid;border-radius: 30rpx;display: flex;
			...
			<view style="width: 90%;height: 54%;display: flex;margin:0 auto;margin-top: 80rpx;background-repeat: no-repeat;background-size: contain;
			background-position: center;
			 border-radius: 8px;" :style="{backgroundImage: 'url('+pictureUrl+')'}"
    ...
    ">
		...
		<view style="grid-row-start: 5; grid-row-end: 7; position: relative" class="items">
          ...
          ">
					...
					<input v-if="nicnNameChanging" v-model.lazy="nickname" style="width: 180rpx;
		...
		<view style="grid-row-start: 7; grid-row-end: 17;position: relative;" class="items">
			...
			<view style="height: 80%;width: 90%;position: relative;left: 0;right: 0;margin: auto;top: 20rpx;" id="main"
				v-if="isShowPicture">
		...
		methods: {
			...
			changeNickname(value) {
				if (value) {
					...
					uni.request({
						url: "http://localhost:3000/changeNickname",
						method: "post",
						data: {
			...
			sureChangePicture(){
				console.log(this.pictureUrl);
				uni.request({
					url:"http://localhost:3000/changePicture",
					method:"POST",
					data:{
						url:this.pictureUrl,
						userId:this.uid
